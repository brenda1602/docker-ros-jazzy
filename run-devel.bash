...
# This script checks if a Docker image named "ubuntu-ros-npd-brazil" exists.
# If the image does not exist, it triggers a Docker Compose build with the options
# to compress the build context and not use any cached layers.
if [ ! "$(docker images | grep "${IMAGE_NAME}"  | awk '{ split($13,a,"/"); print $3 }')" ]; then
    # ${EXEC_DOCKER_COMPOSE} build --compress --no-cache
    docker build --force-rm --compress --no-cache=true -t "${IMAGE_NAME}:$IMAGE_TAG" \
        --build-arg BASE_IMAGE=$BASE_IMAGE \
        --build-arg USERNAME=$USERNAME \
        --build-arg OII_UUID=1000 \
        --build-arg OII_UGID=1000 \
    ...
    if [ ! "$(docker ps -q -f name=${IMAGE_NAME})" ]; then
        if [ ! "$(docker ps -aq -f status=exited -f name=${CONTAINER_NAME})" ]; then
            docker create -it \
                ...
                --volume="${HOME}/.gitconfig:/home/${USERNAME}/.gitconfig" \
                --volume="${HOME}/.git-credentials:/home/${USERNAME}/.git-credentials":rw \
                --name="${CONTAINER_NAME}" \
                --user="${USERNAME}" \
        ...
        docker exec -ti "${IMAGE_NAME}-container" /bin/bash
    fi
